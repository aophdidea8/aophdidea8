...
class lastfmApiGeo extends lastfmApi {
	...
	public function getEvents($methodVars) {
		...
		if ( !empty($methodVars['location']) || ( !empty($methodVars['lat']) && !empty($methodVars['long']) ) ) {
			$vars = array(
			...
			if ( $call = $this->apiGetCall($vars) ) {
                                ...
                                if(isset($call->events['location'])){
                                    $events['location'] = (string) $call->events['location'];
                                }
                                
                                if(isset($call->events['lat'])){
                                    $events['lat'] = (string) $call->events['lat'];
                                }
                                
                                if(isset($call->events['long'])){
                                    $events['long'] = (string) $call->events['location'];
                                }
                                
                                if(isset($call->events['limit'])){
                                    $events['limit'] = (string) $call->events['limit'];
                                }
                                
                                if(isset($call->events['distance'])){
                                    $events['distance'] = (string) $call->events['distance'];
                                }
